...
mod bench_range_proof {
            ...
            move |b, &&m| {
                ...
                let v_vec = (0..m)
                    .map(|_i| {
                        ...
                        let v_fe: GE::Scalar = ECScalar::from(&v);
                        v_fe
                    })
                    .collect::<Vec<GE::Scalar>>();

                let r_vec = (0..m).map(|_i| ECScalar::new_random()).collect::<Vec<GE::Scalar>>();
                ...
                let range_proof =
                    RangeProof::<GE>::prove(&g_vec, &h_vec, &g, &h, v_vec.clone(), &r_vec, n);

                b.iter(|| {
                    let result = RangeProof::<GE>::aggregated_verify(
                        &range_proof,
                        &g_vec,
                        &h_vec,
                        &g,
        ...
        c.bench_function("range proof, n=16, m=2", move |b| {
            ...
            b.iter(|| {
                let range_proof =
                    RangeProof::<GE>::prove(&g_vec, &h_vec, &g, &h, v_vec.clone(), &r_vec, n);
                let result =
                    RangeProof::<GE>::verify(&range_proof, &g_vec, &h_vec, &g, &h, &ped_com_vec, n);
                assert!(result.is_ok());
            })
        });
    }
    ...
    pub fn create_range_proof_16_2(c: &mut Criterion) {
        c.bench_function("create range proof, n=16, m=2", move |b| {
            ...
            b.iter(|| RangeProof::<GE>::prove(&g_vec, &h_vec, &g, &h, v_vec.clone(), &r_vec, n))
        });
    }
    ...
    pub fn verify_range_proof_16_2(c: &mut Criterion) {
        c.bench_function("verify range proof, n=16, m=2", move |b| {
            ...
            let range_proof = RangeProof::<GE>::prove(&g_vec, &h_vec, &g, &h, v_vec.clone(), &r_vec, n);
    ...
    pub fn fast_verify_range_proof_16_2(c: &mut Criterion) {
        c.bench_function("multiexp-based verify range proof, n=16, m=2", move |b| {
            ...
            b.iter(|| {
                let result =
                    RangeProof::<GE>::fast_verify(&range_proof, &g_vec, &h_vec, &g, &h, &ped_com_vec, n);
                assert!(result.is_ok());
            })
        });
    }
...
mod bench_wip_range_proof {
    ...
    use criterion::Criterion;
    use curv::arithmetic::traits::*;
    use curv::elliptic::curves::secp256_k1::GE::Scalar;
    ...
    fn aggregated_wip_rangeproof_helper(n: usize, c: &mut Criterion) {
        ...
        c.bench_function_over_inputs(
            ...
            move |b, &&m| {
                ...
                let KZen: &[u8] = &[75, 90, 101, 110];
                let kzen_label = BigInt::from_bytes(KZen);
                let stmt = GStatementRP::<GE>::generate_bases(&kzen_label, m, n);
                ...
                let r_vec = (0..m).map(|_| ECScalar::new_random()).collect::<Vec<GE::Scalar>>();
                ...
                let range_proof_wip = RangeProofWIP::<GE>::prove(stmt.clone(), v_vec.clone(), &r_vec);

                b.iter(|| {
                    let result = RangeProofWIP::<GE>::aggregated_verify(
                        &range_proof_wip,
                        stmt.clone(),
                        &ped_com_vec,
                    );
    ...
    pub fn create_wip_range_proof_16_2(c: &mut Criterion) {
        c.bench_function("create wip range proof, n=16, m=2", move |b| {
            ...
            b.iter(|| {
                RangeProofWIP::<GE>::prove(stmt.clone(), v_vec.clone(), &r_vec);
            })
        });
    }
    ...
    pub fn verify_wip_range_proof_16_2(c: &mut Criterion) {
        c.bench_function(
            ...
            move |b| {
                ...
                b.iter(|| {
                    let result =
                        RangeProofWIP::<GE>::verify(&range_proof_wip, stmt.clone(), &ped_com_vec);
                    assert!(result.is_ok());
                })
            },
        );
